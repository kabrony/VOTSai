...
class ModelCache:
    ...
    def remove_model(self, name: str) -> bool:
        ...
        with self._lock:
            if name not in self._models:
                logger.warning(f"Attempted to remove non-existent model: {name}")
                return False
                
            # Some models may have a close() method
            try:
    ...
    def clean_cache(self, max_idle_time: Optional[int] = None) -> int:
        """Remove idle models from the cache."""
        with self._lock:
