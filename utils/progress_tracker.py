...
class ProgressTracker:
    ...
    def update_progress(self, operation_id: str, progress: float, message: Optional[str] = None) -> bool:
        ...
        with self._lock:
            ...
            if operation["status"] != "running":
                logger.warning(f"Attempted to update {operation['status']} operation: {operation_id}")
                return False
            ...
            if progress > 0:
                ...
                operation["eta"] = remaining
                
            logger.debug(f"Updated operation {operation_id}: {progress:.1%} - {message or 'No message'}")
            return True
    ...
    def complete_operation(self, operation_id: str, result: Any = None) -> bool:
        ...
        with self._lock:
            ...
            operation = self._operations[operation_id]
            if operation["status"] != "running":
                logger.warning(f"Attempted to complete {operation['status']} operation: {operation_id}")
                return False
    ...
    def fail_operation(self, operation_id: str, error: str) -> bool:
        ...
        with self._lock:
            ...
            operation = self._operations[operation_id]
            if operation["status"] != "running":
                logger.warning(f"Attempted to fail {operation['status']} operation: {operation_id}")
                return False
